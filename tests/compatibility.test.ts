...
describe('flexPairing()', () => {
  // ── Success path tests (6 tests) ─────────────────────────────────────────

  it('returns RuleResult with ruleId flex-pairing on success', async () => {
    ...
    const { flexPairing } = await import('../src/domain/compatibility/flex-pairing.js');
    const result = await flexPairing(makeSetup({ flexRating: 6 }), BASE_RIDER, [], 'test-key');
    expect(result.ruleId).toBe('flex-pairing');
  });

  it('returns advisory: true on success', async () => {
    vi.doMock('@anthropic-ai/sdk', () => ({
      default: class {
        messages = {
          create: vi.fn().mockResolvedValue({
  ...
  it('returns verdict pass when tool_use block is present with valid input', async () => {
    vi.doMock('@anthropic-ai/sdk', () => ({
      default: class {
        messages = {
          create: vi.fn().mockResolvedValue({
  ...
  it('reason string contains the mocked recommendation text', async () => {
    vi.doMock('@anthropic-ai/sdk', () => ({
      default: class {
        messages = {
          create: vi.fn().mockResolvedValue({
  ...
  it('works when board.flexRating is a number (e.g., 6)', async () => {
    vi.doMock('@anthropic-ai/sdk', () => ({
      default: class {
        messages = {
          create: vi.fn().mockResolvedValue({
  ...
  it('works when board.flexRating is undefined (sends unknown to Claude)', async () => {
    vi.doMock('@anthropic-ai/sdk', () => ({
      default: class {
        messages = {
          create: vi.fn().mockResolvedValue({
    ...
    const { flexPairing } = await import('../src/domain/compatibility/flex-pairing.js');
    const result = await flexPairing(makeSetup({}), BASE_RIDER, [], 'test-key');
    expect(result.verdict).toBe('pass');
  });

  // ── Error path tests (4 tests) ────────────────────────────────────────────

  it('returns unknown verdict when SDK throws (network error)', async () => {
    vi.doMock('@anthropic-ai/sdk', () => ({
      default: class {
        messages = {
  ...
  it('returns unknown verdict when response.content has no tool_use block (empty content)', async () => {
    vi.doMock('@anthropic-ai/sdk', () => ({
      default: class {
        messages = {
  ...
  it('returns unknown verdict when response.content has only text blocks (no tool_use)', async () => {
    vi.doMock('@anthropic-ai/sdk', () => ({
      default: class {
        messages = {
          create: vi.fn().mockResolvedValue({
  ...
  it('never throws — resolves with unknown result even on error', async () => {
    vi.doMock('@anthropic-ai/sdk', () => ({
      default: class {
        messages = {
    ...
    const { flexPairing } = await import('../src/domain/compatibility/flex-pairing.js');
    // Must not reject — must resolve to a RuleResult
    const result = await expect(flexPairing(makeSetup({}), BASE_RIDER, [], 'test-key')).resolves.toBeDefined();
    void result;
  });

  // ── No-API tests (3 tests) ────────────────────────────────────────────────

  it('UNKNOWN_RESULT shape: unknown verdict on error has advisory:true', async () => {
    vi.doMock('@anthropic-ai/sdk', () => ({
      default: class {
        messages = {
          create: vi.fn().mockRejectedValue(new Error('Forced error')),
        };
      },
    }));
    const { flexPairing } = await import('../src/domain/compatibility/flex-pairing.js');
  ...
  it('advisory field is always true on success path result', async () => {
    vi.doMock('@anthropic-ai/sdk', () => ({
      default: class {
        messages = {
          create: vi.fn().mockResolvedValue({
    ...
    const { flexPairing } = await import('../src/domain/compatibility/flex-pairing.js');
    const result = await flexPairing(makeSetup({ flexRating: 5 }), BASE_RIDER, [], 'test-key');
    expect(result.advisory).toBe(true);
  });

  it('freestyle ridingStyle smoke test — does not crash', async () => {
    vi.doMock('@anthropic-ai/sdk', () => ({
      default: class {
        messages = {
          create: vi.fn().mockResolvedValue({
    ...
    const { flexPairing } = await import('../src/domain/compatibility/flex-pairing.js');
    const freestyleRider: RiderProfile = { ...BASE_RIDER, ridingStyle: 'freestyle' };
    const result = await flexPairing(makeSetup({ flexRating: 3 }), freestyleRider, [], 'test-key');
    expect(result.ruleId).toBe('flex-pairing');
  });
});
